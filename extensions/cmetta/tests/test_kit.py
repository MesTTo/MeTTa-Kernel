"""Prove the C corpus driver preserves long JSON strings and bounded escapes.

Assumes: argv names the built driver and this worktree's ai-tmp directory.
Guarantees: exits nonzero unless a source above 4 KiB round-trips exactly, a
surrogate pair decodes, and a malformed later source leaves stdout empty.
...
def _require(condition: object, claim: str) -> None:
    if not condition:
        raise AssertionError(claim)


def main() -> int:
    """Run the driver against valid and malformed JSON corpora."""
    if len(sys.argv) != 3:
    ...
    _require(scratch.name.startswith("cmetta-kit-test-"), "unsafe scratch name")
    ...
    try:
        ...
        completed = subprocess.run(  # noqa: S603 -- gate supplies the built test driver.
            [str(driver), str(valid)], capture_output=True, text=True, check=False
        )
        _require(completed.returncode == 0, completed.stderr)
        report = json.loads(completed.stdout)
        _require(report["programs"][0]["source"] == long_source,
                 "the driver truncated a source above 4 KiB")
        _require(report["programs"][0]["answers"][0]["text"] == "2",
                 "the long source did not execute intact")
        _require(report["programs"][1]["source"] == unicode_source,
        ...
        refused = subprocess.run(  # noqa: S603 -- gate supplies the built test driver.
            [str(driver), str(malformed)], capture_output=True, check=False
        )
        _require(refused.returncode == 1,
                 "a source ending in a lone backslash was accepted")
        _require(refused.stdout == b"",
                 "a malformed late source emitted a partial JSON report")
        _require(b"unterminated escape in JSON string" in refused.stderr,
