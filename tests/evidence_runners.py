...
Guarantees:
  ...
  - a declared collector whose anchor has left its runner is reported instead
    of being applied [tested 2026-08-18: tests/check_evidence_selftest.py]
Fails when:
...
COLLECTORS = (
    ...
    Collector(
        ...
        recursive=True,
        excludes=("*/_fixtures/*",),
        skip_file="tests/example_skips.txt",
        skip_anchor="grep -v '^#' tests/example_skips.txt | awk 'NF {print $1}'",
    ),
)
