...
#   Usage: sh tests/upstream_bench.sh [BASE_REF]   (BENCH_RUNS=N to change timing runs)
...
command -v swipl >/dev/null || { echo "upstream_bench.sh: swipl not found" >&2; exit 2; }
git -C "$HERE" rev-parse --verify "$BASE_REF^{commit}" >/dev/null || {
    echo "upstream_bench.sh: base ref $BASE_REF does not resolve" >&2; exit 2; }
