% Purpose: optional type declarations checked at development time. A PlDoc
%   mode line above a clause, `%% f(+X:integer) is det.`, becomes `the(integer,
%   X)` goals at the head of that clause's body; under `swipl -O` the whole
%   mechanism is not loaded and the clause is the unannotated one.
%
...
%     [source: the `:- if(current_prolog_flag(optimise,true)).` below].
%   - a module gets checks only if it imported the/2 from here, which
%     module_wants_mavis/1 tests, so loading this file changes nothing on its
%     own [source: module_wants_mavis/1 below].
% Guarantees:
%   - under -O a clause body is the unannotated body, with no residue
%     [tested: test_the_dev_build_checks_a_planted_type_violation_and_optimise
%     _strips_it] [measured 2026-08-19: `double(X,Y) :- Y is X*2` under
%     `%% double(+X:integer, -Y:integer) is det.` compiles to
%     `the(integer,A),the(integer,B),B is A*2` without -O and to `B is A*2`
%     with it].
% Open Obligations:
%   To Do: None
%   Hacks: None
