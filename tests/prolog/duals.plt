...
%The declared Atom mask reaches the DUAL exactly as it reaches the positive
%path: with (: hh (-> Atom Bool)) the argument arrives written on both sides,
%so the negation of hh at an unevaluated (dbl 5) holds while the negation at
%the covered 10 answers False.
...
test(test_not_provable_honours_the_atom_mask_its_positive_path_honours,
    ...
    Covered == [false],
    %The positive path keeps the written argument, which matches no equation.
