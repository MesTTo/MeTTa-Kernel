...
metta_seq_parse(Side, Parsed) :-
    (   nonvar(Side),
        Side = [_|_]
    ->  metta_seq_parse_items(Side, Parsed)
    ;   Parsed = Side
    ).

metta_seq_parse_items(Items, Parsed) :-
    (   nonvar(Items),
        Items = [Item|Rest]
    ->  metta_seq_parse_item(Item, ParsedItem),
        metta_seq_parse_items(Rest, ParsedRest),
        Parsed = [ParsedItem|ParsedRest]
    ;   Parsed = Items
    ).

metta_seq_parse_item(Item, Parsed) :-
    (   metta_seq_surface_gap(Item, Var, Kind)
    ->  Parsed = '$metta_seg'(Var, Kind)
    ;   metta_seq_parse(Item, Parsed)
    ).
...
metta_seq_present(Pattern) :-
    nonvar(Pattern),
    Pattern = [_|_],
    metta_seq_present_items(Pattern).

metta_seq_present_items(Items) :-
    nonvar(Items),
    Items = [Item|Rest],
    (   metta_seq_surface_gap(Item, _, _)
    ->  true
    ;   metta_seq_present(Item)
    ->  true
    ;   metta_seq_present_items(Rest)
    ).
...
metta_seq_head_plan(Pattern, '$metta_seq'(one_sided(left), Parsed)) :-
    metta_seq_parse(Pattern, Parsed).

metta_seq_body_plan(Body, Parsed) :-
    metta_seq_parse(Body, Parsed).
...
%[tested: tests/prolog/suites/reader/segment_equations.plt; commit=b77e3ce5233e5f6032cfc8546ff83ecf4dc3de87].
metta_seq_head_matches(Pattern, Subject) :-
    copy_term(Pattern-Subject, PatternCopy-SubjectCopy),
    metta_seq_head_plan(PatternCopy, Plan),
    once(metta_seq_head_match(Plan, SubjectCopy)).
...
metta_seq_instantiate(Template, Instantiated) :-
    (   nonvar(Template),
        Template = [_|_]
    ->  metta_seq_instantiate_items(Template, Instantiated)
    ;   Instantiated = Template
    ).

metta_seq_instantiate_items(Items, Instantiated) :-
    (   Items == []
    ->  Instantiated = []
    ;   nonvar(Items),
        Items = [Item|Rest]
    ->  metta_seq_instantiate_items(Rest, Tail),
        (   nonvar(Item),
            Item = '$metta_seg'(Run, Kind)
        ->  (   is_list(Run)
            ->  append(Run, Tail, Instantiated)
            ;   metta_seq_open_gap(Kind, Run, Surface),
                Instantiated = [Surface|Tail]
            )
        ;   metta_seq_instantiate(Item, Head),
            Instantiated = [Head|Tail]
        )
    ;   Instantiated = Items
    ).

metta_seq_open_gap(anon, _, '...') :- !.
metta_seq_open_gap(named, Var, [':seg', Var]).
...
metta_seq_parsed(Parsed) :-
    nonvar(Parsed),
    Parsed = [_|_],
    metta_seq_parsed_items(Parsed).

metta_seq_parsed_items(Items) :-
    nonvar(Items),
    Items = [Item|Rest],
    (   nonvar(Item),
        Item = '$metta_seg'(_, _)
    ->  true
    ;   metta_seq_parsed(Item)
    ->  true
    ;   metta_seq_parsed_items(Rest)
    ).
...
metta_seq_gaps(Parsed, Depth, Gaps, Tail) :-
    (   nonvar(Parsed),
        Parsed = [_|_]
    ->  metta_seq_gaps_items(Parsed, Depth, Gaps, Tail)
    ;   Gaps = Tail
    ).

metta_seq_gaps_items(Items, Depth, Gaps, Tail) :-
    (   nonvar(Items),
        Items = [Item|Rest]
    ->  (   nonvar(Item),
            Item = '$metta_seg'(_, _)
        ...
        ;   Inner is Depth + 1,
            metta_seq_gaps(Item, Inner, Gaps, Mid)
        ),
        metta_seq_gaps_items(Rest, Depth, Mid, Tail)
    ;   Gaps = Tail
    ).
...
metta_seq_ordinary(Parsed, Vars, Tail) :-
    (   var(Parsed)
    ->  Vars = [Parsed|Tail]
    ;   Parsed = [_|_]
    ->  metta_seq_ordinary_items(Parsed, Vars, Tail)
    ;   Vars = Tail
    ).

metta_seq_ordinary_items(Items, Vars, Tail) :-
    (   nonvar(Items),
        Items = [Item|Rest]
    ->  (   nonvar(Item),
            Item = '$metta_seg'(_, _)
        ->  Mid = Vars
        ;   metta_seq_ordinary(Item, Vars, Mid)
        ),
        metta_seq_ordinary_items(Rest, Mid, Tail)
    ;   Vars = Tail
    ).

%The named gap variables among collected occurrences, sharing preserved.
metta_seq_named_vars([], []).
metta_seq_named_vars([g(_, _, Gap)|Gaps], Names) :-
    (   Gap = '$metta_seg'(Var, named)
    ->  Names = [Var|Rest]
    ;   Names = Rest
    ),
...
metta_seq_classify(Left, Right, Case) :-
    ...
    (   Mixed \== []
    ->  metta_seq_refuse(Left, Right, Mixed, mixed_roles)
    ;   RightGaps == []
    ->  Case = one_sided(left)
    ;   LeftGaps == []
    ->  Case = one_sided(right)
    ;   metta_seq_all_final(LeftGaps),
        metta_seq_all_final(RightGaps)
    ->  Case = last_position
    ;   metta_seq_linear_shallow(LeftGaps, RightGaps)
    ->  Case = linear_shallow
...
metta_seq_mixed_roles(Left, Right, LeftGaps, RightGaps, Mixed) :-
    ...
    (   GapVars == []
    ->  Mixed = []
    ;   metta_seq_ordinary(Left, TermVars, RightTermVars),
        metta_seq_ordinary(Right, RightTermVars, []),
        metta_seq_both_roles(GapVars, TermVars, Mixed)
    ).

metta_seq_both_roles([], _, []).
metta_seq_both_roles([Var|Vars], TermVars, Mixed) :-
    (   memberchk_eq(Var, TermVars)
    ->  Mixed = [Var|Rest]
    ;   Mixed = Rest
    ),
    metta_seq_both_roles(Vars, TermVars, Rest).

metta_seq_all_final([]).
metta_seq_all_final([g(_, Final, _)|Gaps]) :-
    Final == true,
    metta_seq_all_final(Gaps).
...
metta_seq_linear_shallow(LeftGaps, RightGaps) :-
    ...
    metta_seq_named_vars(Gaps, Names),
    metta_seq_distinct(Names).

metta_seq_distinct([]).
metta_seq_distinct([V|Vs]) :-
    \+ memberchk_eq(V, Vs),
    metta_seq_distinct(Vs).
...
metta_seq_refuse(Left, Right, Mixed, Reason) :-
    metta_seq_surface(Left, LeftSurface),
    metta_seq_surface(Right, RightSurface),
    metta_seq_surface_items(Mixed, MixedSurface),
    throw(error(metta_seq_outside_fragment(LeftSurface, RightSurface,
                                           MixedSurface, Reason),
                none)).
...
metta_seq_surface(Parsed, Surface) :-
    ...
    ;   nonvar(Parsed),
        Parsed = [_|_]
    ->  metta_seq_surface_items(Parsed, Surface)
    ;   Surface = Parsed
    ).

metta_seq_surface_items(Items, Surface) :-
    (   nonvar(Items),
        Items = [Item|Rest]
    ->  metta_seq_surface(Item, Head),
        metta_seq_surface_items(Rest, Tail),
        Surface = [Head|Tail]
    ;   Surface = Items
    ).
...
%
%A gap pattern reaches the engine WRAPPED, as '$metta_seq'(Plan, Parsed), and
%the wrapper is the whole reason a gap-free program pays nothing. match/4 and
%metta_match_atoms/2 each dispatch on it with one clause whose guard is nonvar/1
...
metta_seq_query_plan(Pattern, '$metta_seq'(Plan, Parsed)) :-
    metta_seq_parse(Pattern, Parsed),
    metta_seq_outcome(metta_seq_admit(Parsed), query, Plan).

metta_seq_outcome(Goal, Witness, Plan) :-
    catch(( call(Goal), Plan = Witness ), error(Why, _), Plan = refused(Why)).

metta_seq_admit(Parsed) :-
    (   nonvar(Parsed),
        Parsed = [Comma|Conjuncts],
        Comma == ','
    ->  metta_seq_admit_each(Conjuncts)
    ;   metta_seq_classify(Parsed, _Subject, _Case)
    ).

metta_seq_admit_each([]).
metta_seq_admit_each([Conjunct|Conjuncts]) :-
    metta_seq_classify(Conjunct, _Subject, _Case),
    metta_seq_admit_each(Conjuncts).

%Each certificate has its own procedure and the dispatcher never guesses.
metta_seq_unify(one_sided(left), Left, Right) :-
    metta_seq_atoms(Left, Right).
metta_seq_unify(one_sided(right), Left, Right) :-
    metta_seq_atoms(Right, Left).
metta_seq_unify(last_position, Left, Right) :-
    metta_seq_last_position(Left, Right).
metta_seq_unify(linear_shallow, Left, Right) :-
    metta_seq_linear(Left, Right).
metta_seq_unify(refused(Why), _, _) :-
...
%have to splice in.
metta_seq_atoms(Pattern, Subject) :-
    (   nonvar(Pattern),
...
metta_seq_items(Pattern, Subject) :-
    ...
    ;   nonvar(Pattern),
        Pattern = [Item|Rest],
        (   nonvar(Item),
            Item = '$metta_seg'(Var, _)
        ->  metta_seq_consume(Var, Rest, Subject)
        ;   nonvar(Subject),
            Subject = [Head|Tail],
            metta_seq_atoms(Item, Head),
            metta_seq_items(Rest, Tail)
        )
    ).
...
metta_seq_consume(Var, After, Subject) :-
    ...
    ->  append(Run, Rest, Subject),
        Var = Run,
        metta_seq_items(After, Rest)
    ;   metta_seq_repeat(Var, Subject, Rest),
        metta_seq_items(After, Rest)
    ).

metta_seq_repeat(Run, Subject, Rest) :-
    (   Run == []
    ->  Rest = Subject
    ;   nonvar(Run),
        Run = [Item|Items],
        nonvar(Subject),
        Subject = [Head|Tail],
        metta_match_atoms(Item, Head),
        metta_seq_repeat(Items, Tail, Rest)
    ).
...
metta_seq_lookup(Var, [Key-Run|Rest], Found) :-
    (   Key == Var
    ->  Found = Run
    ;   metta_seq_lookup(Var, Rest, Found)
    ).
...
metta_seq_store(Var, Run, Store0, Store) :-
    (   Run = [Only],
        nonvar(Only),
        Only = '$metta_seg'(Same, _),
        Same == Var
    ->  Store = Store0
    ;   metta_seq_gap_occurs(Var, Run)
    ->  fail
...
metta_seq_publish(Store) :-
    length(Store, Budget),
    metta_seq_resolve_store(Store, Store, Budget, Values),
    metta_seq_bind_store(Store, Values).

metta_seq_resolve_store([], _, _, []).
metta_seq_resolve_store([_-Run|Rest], Store, Budget, [Value|Values]) :-
    metta_seq_resolve_items(Run, Store, Budget, Value),
    metta_seq_resolve_store(Rest, Store, Budget, Values).

metta_seq_bind_store([], []).
metta_seq_bind_store([Var-_|Rest], [Value|Values]) :-
    Var = Value,
    metta_seq_bind_store(Rest, Values).

metta_seq_resolve_items(Items, Store, Budget, Out) :-
    (   Items == []
    ->  Out = []
    ;   nonvar(Items),
        Items = [Item|Rest]
    ->  metta_seq_resolve_items(Rest, Store, Budget, Tail),
        (   nonvar(Item),
            Item = '$metta_seg'(Var, Kind)
        ->  (   metta_seq_lookup(Var, Store, Run)
            ->  Budget > 0,
                Next is Budget - 1,
                metta_seq_resolve_items(Run, Store, Next, Spliced),
                append(Spliced, Tail, Out)
            ;   ( Kind == anon -> Marker = '...' ; Marker = [':seg', Var] ),
                Out = [Marker|Tail]
            )
        ;   metta_seq_resolve_atom(Item, Store, Budget, Head),
            Out = [Head|Tail]
        )
    ;   Out = Items
    ).

metta_seq_resolve_atom(Atom, Store, Budget, Out) :-
    (   nonvar(Atom),
        Atom = [_|_]
    ->  metta_seq_resolve_items(Atom, Store, Budget, Out)
    ;   Out = Atom
    ).
...
metta_seq_last_atoms(Left, Right, Store0, Store) :-
    (   nonvar(Left),
        Left = [_|_],
        nonvar(Right),
        Right = [_|_]
    ->  metta_seq_last_items(Left, Right, Store0, Store)
...
metta_seq_last_items(Left, Right, Store0, Store) :-
    (   Left == [],
        Right == []
    ->  Store = Store0
    ;   metta_seq_trailing_gap(Left, Var)
    ->  metta_seq_absorb(Var, Right, Store0, Store)
    ;   metta_seq_trailing_gap(Right, Var)
    ->  metta_seq_absorb(Var, Left, Store0, Store)
    ;   nonvar(Left),
        Left = [L|Ls],
        nonvar(Right),
        Right = [R|Rs],
        metta_seq_last_atoms(L, R, Store0, Between),
        metta_seq_last_items(Ls, Rs, Between, Store)
    ).

%A gap in this fragment is always the last item of its list, so seeing one at
%the head means the list is exactly that gap.
metta_seq_trailing_gap(Items, Var) :-
    nonvar(Items),
    Items = [Item|Rest],
    Rest == [],
    nonvar(Item),
...
%same way.
metta_seq_absorb(Var, Items, Store0, Store) :-
    (   metta_seq_lookup(Var, Store0, Stored)
    ->  metta_seq_last_items(Stored, Items, Store0, Store)
    ;   metta_seq_store(Var, Items, Store0, Store)
    ).
...
metta_seq_linear(Left, Right) :-
    (   nonvar(Left),
        ...
        nonvar(Right),
        Right = [_|_]
    ->  metta_seq_linear_items(Left, Right, [], Store),
        metta_seq_publish(Store)
    ;   metta_match_atoms(Left, Right)
    ).

metta_seq_linear_items(Left, Right, Store0, Store) :-
    (   Left == [],
        Right == []
    ->  Store = Store0
    ;   Left == []
    ->  metta_seq_linear_drain(Right, Store0, Store)
    ;   Right == []
    ->  metta_seq_linear_drain(Left, Store0, Store)
    ;   nonvar(Left),
        Left = [L|Ls],
        nonvar(Right),
        Right = [R|Rs],
        metta_seq_linear_step(L, Ls, R, Rs, Store0, Store)
    ).

%One side is exhausted, so every remaining gap on the other takes the empty run
%and every remaining ordinary item refutes.
metta_seq_linear_drain(Items, Store0, Store) :-
    (   Items == []
    ->  Store = Store0
    ;   nonvar(Items),
        Items = [Item|Rest],
        nonvar(Item),
        Item = '$metta_seg'(Var, _),
        metta_seq_project(Var, Store0, Between),
        metta_seq_linear_drain(Rest, Between, Store)
    ).
...
metta_seq_linear_step(L, Ls, R, Rs, Store0, Store) :-
    ...
    ->  (   nonvar(R),
            R = '$metta_seg'(_, _)
        ->  metta_seq_gaps_meet(L, Ls, R, Rs, Store0, Store)
        ;   metta_seq_gap_meets_item(L, Ls, R, Rs, Store0, Store)
        )
    ;   nonvar(R),
        R = '$metta_seg'(_, _)
    ->  metta_seq_gap_meets_item(R, Rs, L, Ls, Store0, Store)
    ;   metta_match_atoms(L, R),
        metta_seq_linear_items(Ls, Rs, Store0, Store)
    ).
...
%gap's slot.
metta_seq_gap_meets_item('$metta_seg'(Var, _), After, Item, Rest, Store0, Store) :-
    (   metta_seq_project(Var, Store0, Between),
        metta_seq_linear_items(After, [Item|Rest], Between, Store)
    ;   Fresh = '$metta_seg'(_, anon),
        metta_seq_store(Var, [Item, Fresh], Store0, Between),
        metta_seq_linear_items([Fresh|After], Rest, Between, Store)
    ).
...
metta_seq_gaps_meet(Left, Ls, Right, Rs, Store0, Store) :-
    Left = '$metta_seg'(LeftVar, _),
    Right = '$metta_seg'(RightVar, _),
    (   LeftVar == RightVar
    ->  metta_seq_linear_items(Ls, Rs, Store0, Store)
    ;   metta_seq_project(LeftVar, Store0, Between),
        metta_seq_linear_items(Ls, [Right|Rs], Between, Store)
    ;   metta_seq_project(RightVar, Store0, Between),
        metta_seq_linear_items([Left|Ls], Rs, Between, Store)
    ;   Fresh = '$metta_seg'(_, anon),
        metta_seq_store(LeftVar, [Right, Fresh], Store0, Between),
        metta_seq_linear_items([Fresh|Ls], Rs, Between, Store)
    ).

metta_seq_project(Var, Store0, Store) :-
    (   metta_seq_lookup(Var, Store0, Stored)
    ->  Stored == [],
        Store = Store0
    ;   Store = [Var-[]|Store0]
...
metta_seq_space(query, Space, Parsed, OutPattern, Result) :-
    (   nonvar(Parsed),
        Parsed = [Comma|Conjuncts],
        Comma == ','
    ->  conjunctive_match(metta_seq_conjunction(Space, Conjuncts),
                          Space, Parsed, OutPattern, Result)
    ;   metta_seq_candidate(Space, Parsed, Candidate),
        metta_seq_atoms(Parsed, Candidate),
        acyclic_term(OutPattern),
        Result = OutPattern
    ).

metta_seq_conjunction(_, []).
metta_seq_conjunction(Space, [Conjunct|Conjuncts]) :-
    (   metta_seq_parsed(Conjunct)
    ->  metta_seq_space(query, Space, Conjunct, conj, conj)
    ;   match(Space, Conjunct, conj, conj)
    ),
    metta_seq_conjunction(Space, Conjuncts).
...
metta_seq_fixed_length(Items, Fixed) :-
    metta_seq_fixed_length_(Items, 0, Fixed).

metta_seq_fixed_length_(Items, Seen, Fixed) :-
    (   nonvar(Items),
        Items = [Item|Rest]
    ->  (   nonvar(Item),
            Item = '$metta_seg'(_, _)
        ->  metta_seq_fixed_length_(Rest, Seen, Fixed)
        ;   Next is Seen + 1,
            metta_seq_fixed_length_(Rest, Next, Fixed)
        )
    ;   Fixed = Seen
    ).
