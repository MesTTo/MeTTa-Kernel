% This seat's control file; see bindings/python/extension.pl for the model.
%
...
% [source: bindings/cetta/cetta.c, cetta_open]. A process that is not the C
% host -- plain swipl, the Python host -- loads nothing and says nothing.

title('MeTTa in C: the engine embedded through libcetta').
needs(predicate('$cetta_present'/0)).
