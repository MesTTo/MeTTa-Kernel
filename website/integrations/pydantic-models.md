...
The integration uses Pydantic 2's `model_fields` and `model_validate(..., by_name=True)` APIs. Pydantic is optional and has no MeTTa packaging extra, so install Pydantic separately before using this path.
