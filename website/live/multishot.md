...
clingo's changing-program vocabulary maps onto the existing space and query surface, and the mapping is small enough to be an example rather than a package module: `extensions/python/examples/integration/multishot_solving.py` builds it in two short classes on the core calls alone. A `Part` is a parameterized program template grounded once per argument tuple, clingo's `#program`. An `External` is a fact whose truth toggles between solves and ends with `release()`, clingo's `#external`, which on an engine with no grounding step is exactly a togglable fact.
