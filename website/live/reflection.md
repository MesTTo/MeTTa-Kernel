...
The Python surface describes itself in the `&metta` space. Registered operations appear as `(op name arity kind)` facts. Functions compiled with `@m.define` appear as `(defined space name)`. Standing queries appear as `(subscription space pattern on)`. Each fact is removed when the operation, definition, or subscription is removed.

Because `&metta` is an ordinary space, MeTTa programs can inspect the Python integration with normal matching. The direction also reverses: a Python standing query on `&metta` can watch for control atoms written by a MeTTa program. The program then steers the integration through the same fact and subscription mechanisms used elsewhere.
