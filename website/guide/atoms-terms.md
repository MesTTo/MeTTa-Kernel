<!--
Purpose: document Atoms, operators, and term building against the current Python surface.
Guarantees:
...
A symbol and a grounded string are different atoms. Use `sym(name)` when a symbol name is not a Python identifier, `var(name)` for a variable, `val(value)` to carry a host object, and `Expression(children)` to assemble an expression from one iterable. `parse(source)` reads one form without evaluating it. An answer list is a multiset whose execution order has no meaning; an `Expression` is object-level data whose child order and multiplicity are preserved exactly.
