<!--
Purpose: introduce the Python seat and route to the sections that document it.
...
Guarantees: every section named here exists in the navigation
  [tested: test_every_site_page_is_reachable_from_the_navigation; commit=057cc60ec553c5820f95ee361f1fad057467f3c3]
-->

# Python

The Python seat is two directions at once, which is why its control file is the
...
the WebAssembly build, a stripped install — cannot host this seat at all, so the
...
- [Guide](../guide/) covers the surface by task: queries, definitions, spaces,
  threads, observability, notebooks.
- [Integrations](../integrations/) are the library boundaries — dataframes,
  DuckDB, Pydantic, arrays, HTTP.
- [Live systems](../live/) cover standing queries, remotes and the event loop.
...
Install it with `pip install .` from the repository root, or point `METTA_PATH`
at a clone. [Install and first steps](../guide/getting-started.md) is the place
to start.
